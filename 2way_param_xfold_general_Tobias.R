...
for (i in beg:end) {
	...
	facPot <- list(linregPotential(dim = c(100, 100)), # PR_overall | EXPR
					...
					linregPotential(dim = c(100, 100), range1 = c(min_p, max_p), range2 = c(min_p,max_p), alpha = 1, beta = 0, var = 1), # PR_i | PR
					linregPotential(dim = c(100, 100), range1 = c(min_gb, max_gb), range2 = c(min_gb,max_gb), alpha = 1, beta = 0, var = 1)) # GB_i | GB
	...
	# begin x-fold here
	for (fold in 1:nfolds) { # iterate through 8 folds
		...
		potentials(dfg_g1_prior) <- c(potentials(dfg_g1), list(betaPotential(range=c(min_e,max_e),alphas=alpha, betas=beta)))
		...
		potentials(dfg_g2_prior) <- c(potentials(dfg_g2), list(betaPotential(range=c(min_e,max_e),alphas=alpha, betas=beta)))
		...
		data_eval <- df_wo_expr[-c(g1_xfold_training,g2_xfold_training),]
		...
		model$auc_training[fold] <- auc(predictor=scores_train,response=c(rep("N",length(g1_xfold_training)),rep("T",length(g2_xfold_training))))
		model$auc_evaluation[fold] <- auc(predictor=scores_eval,response=c(rep("N",(82-length(g1_xfold_training))),rep("T",(730-length(g2_xfold_training)))))

...
for (i in beg:end) {
	...
	df <- data_BRCA[[i]] %>%
		as.data.frame() %>%
		mutate(EXPR = (read_count+1) / lib_size) %>%
		mutate(EXPR = as.integer(cut(EXPR, breaks = seq(min_e,max_e,length.out = 10001), labels = c(1:10000)))) %>%
	...
	# Base models
	varDim <- c(10000,rep(100, 2+n_pr_cpg+n_gb_cpg))
	facPot <- list(linregPotential(dim = c(10000, 100)), # PR_overall | EXPR
					linregPotential(dim = c(10000, 100)), # GB_overall | EXPR
	...
	varDimPrior <- varDim
	facPotPrior <- facPot
	facPotPrior[[5]] <- matrix(1, 1,10000)
	facNbsPrior <- facNbs
	...
	for (fold in 1:nfolds) { # iterate through nfolds folds
		...
		dataList[[1]] <- lapply(1:length(g1_xfold_training), FUN=function(i){
			breaks <- seq(min_e, max_e, length.out = 10001)
		...
		potentials(dfg_g1_prior) <- c(potentials(dfg_g1), list(betaPotential(dim=c(1,10000), range=c(min_e,max_e),alphas=alpha, betas=beta)))
		...
		potentials(dfg_g2_prior) <- c(potentials(dfg_g2), list(betaPotential(dim=c(1,10000), range=c(min_e,max_e),alphas=alpha, betas=beta)))
